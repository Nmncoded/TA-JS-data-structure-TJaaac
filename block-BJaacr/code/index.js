...
let objCombineHouse = {};
for(let i=0; i <= got.houses.length-1; i++ ){
  objCombineHouse[got.houses[i].name] = got.houses[i].people.length
  
}
console.log(objCombineHouse);

// 15. Log the name of all the people of all the houses in got one by one

let arrAllPeople = [];
for(let house of got.houses){
  for(let i = 0 ; i <= house.people.length-1 ; i++ ){
    arrAllPeople.push(house.people[i].name)
  }
}
...
let arrallDescription = [];
for(let house of got.houses){
  for(let i = 0 ; i <= house.people.length-1 ; i++ ){
    arrallDescription.push(house.people[i].description)
  }
}
